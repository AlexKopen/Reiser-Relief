...
export class AboutComponent implements OnInit {
    ...
    navigationActive(navigationSection: NavigationSection): boolean {
        if (!navigationSection.active) {
            ...
            const nextNavigationSection = this.aboutSections[this.aboutSections.findIndex((currentNavigationSection: NavigationSection) => {
                return currentNavigationSection === navigationSection;
            }) + 1];
            return nextNavigationSection === undefined || !nextNavigationSection.active;
        }
    }
}
